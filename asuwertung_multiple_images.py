...
image_files = filedialog.askopenfilenames(parent=root,
                                     title="Please select one or more Images:",
...
def rotate_image(image):
    cv2.namedWindow("Rotate: Press SPACE to rotate, C to cave and cancel",cv2.WINDOW_NORMAL)
    cv2.createTrackbar("Angle","Rotate: Press SPACE to rotate, C to cave and cancel",0,20,nothing)
    switch = '0:CW\nCCW:-'
    cv2.createTrackbar(switch,"Rotate: Press SPACE to rotate, C to cave and cancel",0,1,nothing)
    while(1):
        s = cv2.cv2.getTrackbarPos(switch,"Rotate: Press SPACE to rotate, C to cave and cancel")
        rot = cv2.getTrackbarPos("Angle","Rotate: Press SPACE to rotate, C to cave and cancel")
        if s == 0:
            rotated = imutils.rotate_bound(image,rot)
        else:
            rotated = imutils.rotate_bound(image,-rot)
        cv2.imshow("Rotate: Press SPACE to rotate, C to cave and cancel", rotated)
...
for i in range(len(image_list1)):
    while(True):
        windowName = "Crop image: press SPACE after selection"
        ...
        cv2.imshow("Press SPACE for next image, ESC for same image",imCrop1)
...
for i in range(len(image_list)):
    while(True):
        ...
        windowName = "Select ROI: Press SPACE to save the image selection"
        cv2.namedWindow(windowName,cv2.WINDOW_NORMAL)
...
for i in range(len(image_roi_list)):
    ...
    cv2.createTrackbar("Set threshold", "Set threshold: press 'SPACE' to save and exit the threshold window. Default is 200",200,255,nothing1)   
    while(1):
...
if os.path.exists(directory_name+'/Schilffbuild_auswertung_'+timestr+'.xlsx'):
    workbook = xlsxwriter.Workbook(directory_name+'/Schilffbuild_auswertung_'+timestr+'_1'+'.xlsx')
else:
    workbook = xlsxwriter.Workbook(directory_name+'/Schilffbuild_auswertung_'+timestr+'.xlsx')
